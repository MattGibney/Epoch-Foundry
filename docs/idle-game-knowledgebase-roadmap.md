...
- `Credits`: primary spend resource; generated continuously.
...
- Resets run resources and run upgrades.
...
- Numbers: decimal library or fixed scientific notation utility.
...
- Use delta-time accumulation for idle consistency.
- Clamp large deltas to prevent runaway jumps.
- Save cadence: every 10-20 seconds plus lifecycle events.
...
- Implement state model and save/load.
- Implement core tick loop and resource production.
- Add debug panel for setting resources and fast-forward.

Exit criteria:

- Stable ticking, saving, and restore behavior.

## 7.2 Phase 1: Core Loop (Day 3-5)

- Implement 3 generator tiers with scaling costs.
- Implement buy 1 / buy 10 / buy max.
- Implement run upgrades.
- Add main production UI.
...
- Implement Reboot reset flow and Influence rewards.
- Implement permanent tree nodes and unlock logic.
- Add prestige panel and tree UI.

Exit criteria:

- Reboot loop functional and clearly accelerates future runs.
...
- Add migration-aware save versioning.
...
- Deterministic tick simulation with delta handling.
- Save/load with schema versioning and migration hooks.
- Generator purchase and production formulas.
- Reboot reset and Influence payout.
...
- Stats page (run time, total gained, best reboot).
...
- Make growth legible: always show current gain/sec per key resource.
- Reduce friction: keep primary actions one click away.
- Surface reset value: show projected Influence before Reboot.
- Always explain blockers: "Need X Components to unlock Labs."
- Prefer panel-based layout with sticky resource header.
...
1. Lock this MVP scope and success criteria.
2. Stand up project scaffold and data tables.
3. Implement simulation + save/load first.
4. Ship internal prototype targeting first Reboot in under 2 hours.
